...
const argv = yargs
  ...
  .option('srec', {
    alias: 's',
    description: 'Srec File to Flash',
    type: 'string'
  })
  .option('rt0s', {
    alias: 'r',
  ...
  .option('product', {
    description: 'Product to generata MQTTSN support',
    type: 'string'
  })
  .option('build', {
    description: 'Build type generate MQTTSN support',
    type: 'string'
  })
  .option('term', {
    alias: 't',
...
if (!argv.product) {
  console.log("Must Define Product");
  process.exit(-1)
}

if (!argv.build) {
  console.log("Must Define Build");
  process.exit(-1)
}

var schema_fn = `src/${argv.product}/${argv.product}_${argv.build}_mqtt.json5`
...
if (argv.rt0s) {
  ...
  console.log('Using rt0s as ', argv.rt0s);
}
...
main = async (pfn) => {
  console.log("Renesas Bootloader", pfn);
  const port = new SerialPort(pfn, {
    baudRate: 9600,
    parity: 'none',
    stopBits: 1
  });
  ...
  console.log("Activate MD and press RESET...");
        ...
        if (err) {
          switch (err) {
            ...
            case "CRC":
              ...
              console.log("up crc:", msg, dump("hdlc", 0, frame));
              break;
          }
        } else {
          //dump("frame", 0, frame)
          msg = mqttsn.decode(frame)
          mq.publish(`/data/${argv.id}`, JSON.stringify(msg))
          if (msg.topic == "ping") {
            console.log(`from device PING ${msg.seq}:`, a2s(msg.data));
          ...
          if (msg.rseq) {
            if (reqs[msg.rseq]) {
              ...
              console.log("stray up-message??", msg.rseq, reqs[msg.rseq], reqs);
            }
          }
        }
      })
  ...
  if (mq) {
    ...
    mq.registerAPI("test", "Test mq", ["addr", "len"], (msg) => {
      var a = parseInt(msg.req.args[1])
      var l = parseInt(msg.req.args[2])
      console.log("Test:", a, l);
      // cmd(sprintf("r%02X%08X", l, a), (err, data) => {
      //   console.log("got", err, data);
      //   mq.reply(msg, data)
      // })
      return {ok:123}
    })
    for (const [key, value] of Object.entries(schema.messages)) {
      if (value.direction != 'up') {
        console.log(`msg '${key}':`, value);
        args = []
        for (var p of value.payload)
          args.push(p.name)
        mq.registerAPI(key, value.descr, args, (msg) => {
          // var a = parseInt(msg.req.args[1])
          // var l = parseInt(msg.req.args[2])
          // console.log("Read:", a, l);
          // cmd(sprintf("r%02X%08X", l, a), (err, data) => {
          //   console.log("got", err, data);

          var [id, s] = mqttsn.encode({
            topic: msg.req.args[0],
            address: msg.req.args[1],
            data_len: msg.req.args[2],
          })
          console.log("got api:", msg, id, s);
