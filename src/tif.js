...
do_cmd_help = () => {
  for (c of Object.keys(COMMANDS).sort()) {
    if (COMMANDS[c].hide)
      continue
    var s = c + " "
    for (a of COMMANDS[c].args) {
      if (a.hide)
        continue
...
main = async (pfn) => {
  ...
  check_buf = () => {
    ...
    if (cmd_in_buf[0] == NACK && blen) {
      ...
      if (blen == len + 1) {
        ...
        var p = 0, obj = { raw: ret }
        for (r of cmd_in_progress.ret) {
          if (r.size == 1) {
            if (r.array) {
              obj[r.n] = []
              for (i = p; i < ret.length; i++) {
                obj[r.n].push(ret[p])
                p += 1
              }
            } else {
  ...
  mq.registerAPI('send', "Sends a message to DUT", [], (msg) => {
    ...
    frame = hdlc.send(Buffer.from(buf))
  ...
  mq.registerAPI('hdlc_status', "Get HDLC Status", [], (msg) => {
    return { active: hdlc_on, pings: hdlc_pings, stamp: hdlc_ping_stamp };
  });
  mq.registerAPI('ping', "Ping and get HDLC Status", [], (msg) => {
    return { active: hdlc_on, pings: hdlc_pings, stamp: hdlc_ping_stamp };
  });
  mq.registerAPI('mod_flash', "Flash a Module", ['srec'], async (msg) => {
