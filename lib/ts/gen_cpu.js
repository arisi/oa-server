...
class Builder {
    constructor() {
        ...
        this.bheader = (jsons, path, comments = false) => {
            var prod = JSON5.parse(fs.readFileSync(jsons).toString());
            var fn = prod.cpu;
            var fn2 = (0, path_1.join)(__dirname, `${this.cpu_path}/${fn}/${fn}.json5`);
            var conf = JSON5.parse(fs.readFileSync(fn2).toString());
            conf.modules = {};
            for (var f of conf.files) {
                fn2 = (0, path_1.join)(__dirname, `${this.cpu_path}/${fn}/${f}.json5`);
          ...
          printf("%-20.20s: 0x%08X OK\\n","${name}",&(${name}));\n`;
                            ...
                            var key;
                            var dd2;
                            for ([key, dd2] of Object.entries(r.d).sort((a, b) => {
